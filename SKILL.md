...
4. Render the digest.
   ...
   - For a configured WeChat Official Account, use `scripts/upload_wechat_draft.py` only after the article and cover exist and the user authorizes draft creation. The script converts Markdown to the skill's validated, WeChat-safe editorial HTML; do not paste raw Markdown or depend on the editor to render it. Create a draft by default and report its `media_id` for the operator to publish manually in the backend. Pass `--publish` only when the account's `freepublish/submit` permission has been verified in the WeChat Developer Platform; a `48001 api unauthorized` response means the account lacks this capability, not that the article needs to be regenerated. Keep `WECHAT_APP_SECRET` in ignored local configuration or a secret manager.
   ...
   - **Automatic weekly draft mode:** when the account lacks verified `freepublish/submit` permission, create a draft, then rebuild, commit, and push the reviewed public site. If draft creation fails, stop before changing the public site. Report the draft `media_id` and the separate site commit/Pages status; the operator may manually publish the draft afterward.
   - **Automatic weekly-release mode:** use `scripts/release_weekly_issue.py generated/YYYYMMDD/article.md generated/YYYYMMDD/cover.png` only after the account has verified `freepublish/submit` permission. It submits the Official Account publication first, then builds, commits, and pushes the GitHub Pages archive. If the WeChat submission fails, stop before changing the public site; if site deployment fails after submission, report the two outcomes separately. A WeChat `publish_id` means submission succeeded, not necessarily that platform review has completed.
